        ...
        <div class="stay-preview-info">
          <div class="flex card-rate">
            <span class="preview-rate">
              ...
              <span class="total-rate" style="color:black">  {{totalAverage}}  </span>
          </div>
...
export default {
  ...
  created() {
    ...
    if(user){
      ...
      const isLiked = user.likedStays.length > 0 ? user.likedStays.some((stay) => stay._id === stayId) : undefined;
      this.liked = isLiked
    }
  ...
  computed: {
    averageCleanliness() {
      return (this.stay.reviews.reduce((sum, review) => sum + review.rate.Cleanliness, 0) / this.stay.reviews.length).toFixed(1) || 4.7;
    },
    averageCommunication() {
      return (this.stay.reviews.reduce((sum, review) => sum + review.rate.Communication, 0) / this.stay.reviews.length).toFixed(1) || 4.7;
    },
    averageAccuracy() {
      return (this.stay.reviews.reduce((sum, review) => sum + review.rate.Accuracy, 0) / this.stay.reviews.length).toFixed(1) || 4.7;
    },
    averageCheckIn() {
      return (this.stay.reviews.reduce((sum, review) => sum + review.rate.CheckIn, 0) / this.stay.reviews.length).toFixed(1) || 4.7;
    },
    averageLocation() {
      return (this.stay.reviews.reduce((sum, review) => sum + review.rate.Location, 0) / this.stay.reviews.length).toFixed(1) || 4.7;
    },
    averageValue() {
      return (this.stay.reviews.reduce((sum, review) => sum + review.rate.Value, 0) / this.stay.reviews.length).toFixed(1) || 4.7;
    },
    totalAverage() {
      return ((+this.averageCleanliness + +this.averageCommunication + +this.averageAccuracy + +this.averageCheckIn +
        +this.averageLocation + +this.averageValue) / 6).toFixed(2)|| 4.7
    },
    isLiked() {
